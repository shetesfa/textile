...
if ($weekStart > $todayDate) {
    flash('warning', 'Future attendance is not available.');
    header("Location: attendance.php"); exit;
}

// Save — writer only writes today's column, 3 states: present / absent / unset (skip)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save') {
    ...
    foreach ($employees as $emp) {
        ...
        $val   = $_POST[$key] ?? 'unset'; // 'present' | 'absent' | 'unset'
        if ($val === 'unset') continue;   // don't write unset rows
    ...
    flash('success', "Today's attendance saved successfully.");
...
// Load week attendance — NULL means never recorded (show empty)
$attRows = [];
...
<div class="row g-3 mb-4">
  ...
  <div class="col-6 col-lg-3">
    <div class="att-stat-card att-stat-amber">
      <div class="att-stat-icon"><i class="bi bi-pencil-square"></i></div>
      <div class="att-stat-val"><?= round(($presentToday / max(1, $totalEmp)) * 100) ?>%</div>
...
<!-- ── Writer Notice ── -->
<div class="att-writer-notice mb-4">
  <i class="bi bi-info-circle-fill"></i>
  <div>
    <strong>Today's Attendance — <?= date('l, d M Y') ?></strong><br>
    <span>Tap a cell: <strong>empty → P (Present) → A (Absent) → empty</strong>. Past days show recorded status only. Future is locked.</span>
  </div>
</div>
...
<form method="POST" id="attForm">
  ...
  <div class="att-section-card mb-4">
    <div class="att-section-head">
      ...
      <div class="d-flex gap-2 align-items-center flex-wrap">
        <button type="button" class="att-quick-btn att-quick-all-present" onclick="markAllToday('present')">
          <i class="bi bi-check-all"></i> All Present
        </button>
        <button type="button" class="att-quick-btn att-quick-all-absent" onclick="markAllToday('absent')">
  ...
  <div class="att-legend mb-3">
    <span class="att-cell-badge att-cell-none" style="width:auto;padding:0 10px">Empty</span> Not yet marked &nbsp;·&nbsp;
    ...
    <i class="bi bi-lock-fill text-muted"></i> Locked
  </div>
  ...
  <div class="att-section-card">
    <div class="att-section-head">
      ...
      <small class="text-muted">Only <strong>today</strong> is editable — tap cell to cycle status</small>
    </div>
    ...
    <div class="table-responsive">
      <table class="att-table">
        ...
        <tbody>
        <?php if (!$employees): ?>
          <tr><td colspan="9" class="att-empty">No employees found.</td></tr>
        ...
        ?>
          <tr class="att-row">
            ...
            <?php foreach ($weekDates as $d):
              $key      = 'att_' . $emp['id'] . '_' . str_replace('-', '', $d);
              // $recorded is null = never saved, 'present' or 'absent' = saved
              $recorded = $attRows[$emp['id']][$d] ?? null;
              $isToday  = ($d === $todayDate);
            ...
            ?>
              ...
              <?php if ($isToday): ?>
                <!-- TODAY: 3-state interactive cell -->
                <td class="att-td-cell att-td-today att-td-clickable"
              ...
              <?php else: ?>
                <!-- PAST: read-only, show only if recorded -->
                <td class="att-td-cell att-td-past">
                  <?php if ($recorded !== null): ?>
                    <span class="att-cell-badge <?= $recorded === 'present' ? 'att-cell-present' : 'att-cell-absent' ?>">
                      <?= $recorded === 'present' ? 'P' : 'A' ?>
                    </span>
                  <?php else: ?>
    ...
    <div class="att-table-footer">
      <span class="text-muted" style="font-size:.82rem"><?= $totalEmp ?> employees &middot; Only today editable</span>
      <div class="d-flex gap-2 flex-wrap">
        ...
        <button type="submit" class="btn btn-success px-5">
          <i class="bi bi-save me-1"></i> Save Today's Attendance
        </button>
      </div>
    </div>
    <?php endif; ?>
...
.att-writer-notice{
  ...
  color:#1e40af;font-size:.88rem;line-height:1.6;
}
.att-writer-notice i{font-size:1.2rem;margin-top:1px;flex-shrink:0;}

/* ── Legend ── */
.att-legend{
  display:flex;align-items:center;gap:8px;flex-wrap:wrap;
  font-size:.82rem;color:var(--muted);padding:0 2px;
}
...
.att-quick-all-present{background:#d1fae5;color:#065f46;border-color:#a7f3d0;}
.att-quick-all-present:hover{background:#a7f3d0;}
.att-quick-all-absent{background:#fee2e2;color:#991b1b;border-color:#fca5a5;}
.att-quick-all-absent:hover{background:#fca5a5;}
...
/* TODAY cell — clickable, highlighted */
...
/* PAST cell — read-only neutral */
.att-td-past{background:#fafafa;}

/* FUTURE cell — locked gray */
.att-td-future{background:#f9fafb;opacity:.55;cursor:not-allowed;}
...
/* Unset/empty = very light dashed ring, no text */
.att-cell-none{
...
/* Past empty dash */
.att-cell-empty{color:#d1d5db;font-size:.85rem;}
...
.att-days-chip{
  ...
  font-size:.82rem;font-weight:700;min-width:32px;text-align:center;
  background:#f1f5f9;color:#334155;transition:background .2s,color .2s;
}
...
.att-table-footer{
  ...
  display:flex;align-items:center;justify-content:space-between;
  flex-wrap:wrap;gap:10px;background:#f8fafc;font-size:.85rem;
}
...
@media(max-width:768px){
  ...
  .att-writer-notice{font-size:.82rem;padding:12px 14px;}
...
<script>
// States cycle: unset → present → absent → unset
...
function cycleCell(td) {
  ...
  // Update days counter for this employee
...
function updateDaysChip(empId) {
  ...
  // Also count present badges in past cells (read-only)
  const pastBadges = row.querySelectorAll('.att-td-past .att-cell-present');
  days += pastBadges.length;
...
// Init days chips on page load
document.querySelectorAll('[data-emp]').forEach(td => {
  updateDaysChip(td.dataset.emp);
});
</script>
